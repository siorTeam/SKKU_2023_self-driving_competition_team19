...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (!avoiding) { // 회피중이 아닐 때
      if (distance2 < 150 ) { // 장애물 1 만남 -> 커맨드 1 생성, 기본 넘버는 2
        speed = 0;
        HandleBackMotor();
        delay(1000);
        avoiding = true; //회피 시작
    ...
    if (avoiding == true and avoide_num == 1) {
      ...
      if (avoidingTime1 < 15000) {    // 15초 좌
        // 왼쪽으로 회피
        setpoint = (N_setpoint + chng_max); 
        HandleFrontMotor();
        speed = 50;
        HandleBackMotor();
      } else if (avoidingTime1 < 25000) { // 뒤로 10
        setpoint = CalculateSetpoint(W_input);
        HandleFrontMotor();
        speed = 50;
        analogWrite(motor_b_l_1, 0);
        analogWrite(motor_b_l_2, speed);
        analogWrite(motor_b_r_1, 0);
        analogWrite(motor_b_r_2, speed);
      } else if (avoidingTime1 < 30000){ //5정지
        setpoint = N_setpoint;
        HandleFrontMotor();
        speed = 0;
        HandleBackMotor();
        ...
        speed = 50;
        HandleBackMotor();
      } else if (avoidingTime1 < 56000){ // 15 우회전
        setpoint = (N_setpoint - chng_max); 
        HandleFrontMotor();
        speed = 50;
        HandleBackMotor();
      } else if(avoidingTime1 < 150000) {    // 나머지 기본주행
        setpoint = N_setpoint; 
        HandleFrontMotor();
        speed = 50;
        HandleBackMotor();
